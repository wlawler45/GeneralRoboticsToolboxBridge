...
namespace TestGeneralRoboticsToolboxNET
{
    ...
    public class Robot
    {
                 ...
                 Matrix<double> r_tool = default(Matrix<double>), Vector<double> p_tool = default(Vector<double>), string[] joint_names = default(string[]), string root_link_name = default(string), string tip_link_name = default(string))
        {
            ...
            {
                if (!(AlmostEquals(column.L2Norm(),1))) throw new ArgumentException(String.Format("Matrix H is not Acceptable"));
            }
    ...
    public class Transform
    {
        ...
        public static bool operator ==(Transform tran1, Transform tran2)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; i < 3; i++)
                {
                    if (!(AlmostEquals(tran1.R[i, j],tran2.R[i, j]))) return false;
                }
            }
            for (int i = 0; i < 3; i++)
            {
                if (!(AlmostEquals(tran1.P[i], tran2.P[i]))) return false;
            }
            return true;
